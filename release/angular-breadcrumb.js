...
(function(window, angular, undefined) {
    ...
    function $Breadcrumb() {
        ...
        this.$get = ['$state', '$stateParams', '$rootScope', function($state, $stateParams, $rootScope) {
            ...
            registerListenerOnce('$Breadcrumb.$stateChangeSuccess', $rootScope, '$stateChangeSuccess',
                function(event) {
            ...
            return {
                ...
                getStatesChain: function(exitOnFirst) { // Deliberately undocumented param, see getLastStep
                    ...
                    for (var stateRef = $state.$current.self.name; stateRef; stateRef =
                        $$breadcrumbParentState(stateRef)) {
        ...
        return {
            ...
            link: {
                post: function postLink(scope) {
                    ...
                    registerListenerOnce('BreadcrumbDirective.$stateChangeSuccess', $rootScope,
                        '$stateChangeSuccess',
                        function(event) {
                            if (!event.targetScope.ncyBreadcrumbIgnore) {
                                renderBreadcrumb();
            ...
            compile: function(cElement, cAttrs) {
                ...
                return {
                    post: function postLink(scope) {
                        ...
                        var registerWatchersText = function(labelWatcherArray, interpolationFunction,
                            viewScope) {
                            angular.forEach(getExpression(interpolationFunction), function(expression) {
                                var watcher = viewScope.$watch(expression, function(newValue,
                                    oldValue) {
