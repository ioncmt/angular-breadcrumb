...
(function(window, angular, undefined) {
  ...
  function $Breadcrumb() {
    ...
    this.$get = ['$state', '$stateParams', '$rootScope', '$injector', function($state, $stateParams, $rootScope,
      $injector) {
      ...
      var $$breadcrumbParentState = function(stateRef) {
        ...
        if (conf.ncyBreadcrumb && conf.ncyBreadcrumb.parent) {
          //As we are modifying the content of parent in case of DI.
          var parent = angular.copy(conf.ncyBreadcrumb.parent);
          // Handle the "parent" property of the breadcrumb, override the parent/child relation of the state
          var isFunction = angular.isFunction(parent);
          var isAngularDefinition = angular.isArray(parent) && angular.isFunction(parent[parent.length -
            1]);
