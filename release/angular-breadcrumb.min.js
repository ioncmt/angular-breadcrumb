...
! function(a, b, c) {
  ...
  function g() {
    ...
    this.setOptions = function(c) {
      b.extend(a, c)
    }, this.$get = ["$state", "$stateParams", "$rootScope", "$injector", function(g, h, i, j) {
      var k = i;
      f("$Breadcrumb.$stateChangeSuccess", i, "$stateChangeSuccess", function(a) {
        !a.targetScope.ncyBreadcrumbIgnore && d(a.targetScope.$id, k.$id) && (k = a.targetScope)
      });
      var l = function(a) {
        ...
        m = function(b, c) {
          for (var d, f, i = e(c), j = !1, l = !1, m = 0, n = b.length; n > m; m += 1)
            if (b[m].name === i.state) return;
          d = g.get(i.state), d.ncyBreadcrumb && (d.ncyBreadcrumb.force && (j = !0), d.ncyBreadcrumb.skip && (l = !
            0)), d["abstract"] && !a.includeAbstract && !j || l || (i.paramExpr && (f = k.$eval(i.paramExpr)), d.ncyBreadcrumbLink =
            g.href(i.state, f || h || {}), d.ncyBreadcrumbStateRef = c, b.unshift(d))
        },
        n = function(a) {
          var c = e(a),
            d = g.get(c.state);
          if (d.ncyBreadcrumb && d.ncyBreadcrumb.parent) {
            var f = b.copy(d.ncyBreadcrumb.parent),
              h = b.isFunction(f),
              i = b.isArray(f) && b.isFunction(f[f.length - 1]),
              m = h ? f : function() {
                return f
              };
            m = i ? f.pop() : m, m.$inject = i ? f : [k];
            var n = h ? m.apply(this, m.$inject) : j.invoke(m);
            if (n) return n
          }
          return l(d)
        };
      return {
        ...
        getStatesChain: function(b) {
          for (var c = [], d = g.$current.self.name; d; d = n(d))
            if (m(c, d), b && c.length) return c;
          return a.prefixStateName && m(c, a.prefixStateName), c
        },
    ...
    return {
      ...
      link: {
        post: function(e) {
          ...
          f("BreadcrumbDirective.$stateChangeSuccess", d, "$stateChangeSuccess", function(a) {
      ...
      compile: function(d, e) {
        ...
        return g && d.html(g), {
          post: function(d) {
            ...
            f("BreadcrumbLastDirective.$stateChangeSuccess", c, "$stateChangeSuccess", function(a) {
      ...
      compile: function(e, g) {
        ...
        return {
          post: function(e) {
            ...
            f("BreadcrumbTextDirective.$stateChangeSuccess", d, "$stateChangeSuccess", function(a) {
  ...
  h.$inject = ["$interpolate", "$breadcrumb", "$rootScope"], i.$inject = ["$interpolate", "$breadcrumb", "$rootScope"],
    j.$inject = ["$interpolate", "$breadcrumb", "$rootScope"], b.module("ncy-angular-breadcrumb", ["ui.router.state"]).provider(
      "$breadcrumb", g).directive("ncyBreadcrumb", h).directive("ncyBreadcrumbLast", i).directive("ncyBreadcrumbText",
      j)
